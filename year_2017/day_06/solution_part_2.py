...
def count_redistributions_cycles(memory_banks: List[int]) -> int:
    """
    ...
    return len(redistributions) - redistributions.index(memory_banks)


@pytest.mark.parametrize('memory_banks, expected_redistributions', [
    ('0 2 7 0', 4),
    # my puzzle input
    (MY_PUZZLE_INPUT, 1695)
])
def test_count_redistribution_cycles(memory_banks, expected_redistributions):
    """Test the solution"""
    # given
    parsed_input = parse_input(memory_banks)

    # when
    actual_redistributions = count_redistributions_cycles(parsed_input)
